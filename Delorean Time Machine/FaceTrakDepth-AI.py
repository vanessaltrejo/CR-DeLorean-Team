...
class StereoInference:
    ...
    def calc_spatials(self, coords, depth):
        ...
        z_mm = depth
        x_mm = z_mm * math.tan(angle_x)
        y_mm = -z_mm * math.tan(angle_y)

        print(f"x {x_mm} mm, y {y_mm} mm, z {z_mm} mm")

        if x_mm <= -200:
            message = b"A"  # Prefijo b indica que es una cadena de bytes
        elif x_mm >= 400:
            message = b"C"
        elif -199 <= x_mm <= 399 and 400 <= z_mm <= 600 and -50 <= y_mm >= 150:
            message = b"B"
        elif z_mm < 400:
            message = b"D"
        elif z_mm > 600:
        ...
        # Convierte x e y de milímetros a metros
        x_meters = x_mm / 1000
        y_meters = y_mm / 1000
        z_meters = z / 1000  # Convierte profundidad de milímetros a metros

        print(f"x {x_meters:.2f}, y {y_meters:.2f}, z {z_meters:.2f}")  # Imprime valores en metros
